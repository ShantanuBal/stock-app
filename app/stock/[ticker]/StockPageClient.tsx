...
export default function StockPageClient({ ticker, details }: Props) {
  ...
  const weightedShares = details?.weightedSharesOutstanding ?? null;
  const marketCap = price != null && weightedShares != null ? price * weightedShares : null;
  const pe = marketCap != null && details?.netIncome && details.netIncome > 0 ? marketCap / details.netIncome : null;
  const peg = pe != null && details?.epsGrowth != null && details.epsGrowth > 0 ? pe / details.epsGrowth : null;
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            {details?.name && <p className="text-base text-gray-500 dark:text-gray-400">{details.name}</p>}
          </div>
          {price != null && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-5">
        {[
          { label: "Market Cap", value: marketCap != null ? fmtMarketCap(marketCap) : null },
          { label: "P/E Ratio", value: pe != null ? `${pe.toFixed(1)}x` : null },
          { label: "PEG Ratio", value: peg != null ? peg.toFixed(2) : null, highlight: peg != null && (peg < 0 || peg > 3) },
          { label: "Beta", value: beta !== undefined ? (beta != null ? beta.toFixed(2) : "—") : null },
          { label: "Shares Outstanding", value: weightedShares != null ? fmtShares(weightedShares) : null },
          { label: "Net Income", value: details?.netIncome != null ? fmtMarketCap(details.netIncome!) : null },
        ].map(({ label, value, highlight }) => (
          <div key={label} className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">{label}</p>
            <p className={`text-sm font-semibold tabular-nums ${highlight ? "text-red-500" : "text-gray-900 dark:text-white"}`}>
              {value ?? <span className="inline-block h-3 w-12 rounded bg-gray-100 dark:bg-gray-800 animate-pulse" />}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        {details?.description && (
          <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed mb-3">{details.description}</p>
        )}
        <div className="flex gap-4">
          ...
          <a href={`https://en.wikipedia.org/wiki/${encodeURIComponent(details?.name ?? ticker)}`} target="_blank" rel="noopener noreferrer" className="text-xs text-emerald-500 hover:text-emerald-400 transition-colors">
            Wikipedia ↗
          </a>
        </div>
      </div>
